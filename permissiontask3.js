...
const uniqueGroupNames = [...new Set(permissions.map((item) => item.split('.')[1]))]

const mutatedPermissions = uniqueGroupNames.map((groupName) => ({
    groupName,
    permissions: permissions.filter((item) => item.split('.')[1] === groupName)
}))
